...
if(idx.jack!=0){
    switch(id,
        ...
        {fit.result <- fit.model(data=data.fit.sum[-idx.jack],loc=coord.grid,init=init,fixed=c(F,F,F,F,rep(F,nrow(basis.centers))),model="logskew",maxit=1e6,FUN=vario.func2,basis=basis,alpha.func=alpha.func,ncores=ceiling(ncores/2),method=method,lb=lb,ub=ub,opt=TRUE,idx.para=idx.para,pareto=TRUE,partial=TRUE,step2=TRUE,trace=TRUE)},
