...
for(i in 1:nrow(par.skew.normal)){
    ...
    fit.logskew.comp[[i]] <- MCLE(data=samples.skew.normal1[[i]],init=init[1:2],fixed=c(F,F),loc=coord,FUN=cov.func,index=all.pairs[,pairs.idx],model="BR",lb=lb[1:2],ub=ub[1:2],ncores=ncores,maxit=1000,trace=FALSE)
    
    fit.logskew.comp2[[i]] <- MCLE(data=samples.skew.normal2[[i]],init=init[1:2],fixed=c(F,F),loc=coord,FUN=cov.func,index=all.pairs[,pairs.idx],model="BR",lb=lb[1:2],ub=ub[1:2],ncores=ncores,maxit=1000,trace=FALSE)
}
