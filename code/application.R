...
results42 <- mcmapply(1:nrow(data),function(i){fit.model(data=maxima.frechet,loc=loc.sub.trans,init=results4$par[idx.para],fixed=c(F,F),thres=thres,model="BR",maxit=1000,FUN=vario.func,ncores=ncores,method="Nelder-Mead",lb=c(0.01,0.0),ub=c(Inf,1.99),hessian=FALSE,opt=TRUE,trace=TRUE,idx.para=1:2)},mc.cores=ncores,mc.set.seed = TRUE) 
