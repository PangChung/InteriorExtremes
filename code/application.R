...
idx.centers = unlist(lapply(quantile(loc.sub.trans[,1],seq(0.2,0.8,length.out=2)),function(x){ idx = abs(loc.sub.trans[,1] - x) < 5; which(idx)[which.min(abs(loc.sub.trans[idx,2] - median(loc.sub.trans[idx,2])))]}))
...
if(idx.jack!=0){
    switch(id,
     ...
     switch(id,
        ...
        # fit.result <- fit.model(data=data.fit.sum,loc=loc.sub.trans,init=fit.result$par,fixed=c(F,F,rep(T,ncol(basis))),model="logskew",maxit=1000,FUN=vario.func,basis=basis,alpha.func=alpha.func,ncores=ncores,method=method,lb=lb,ub=ub,opt=TRUE,idx.para=idx.para,pareto=TRUE,partial=TRUE,step2=FALSE,trace=TRUE);fit.result$time = proc.time() - t0
        },
        {#t0 <- proc.time();
        ...
        # fit.result <- fit.model(data=data.fit.max,loc=loc.sub.trans,init=fit.result$par,fixed=c(F,F,rep(T,ncol(basis))),model="logskew",maxit=1000,FUN=vario.func,basis=basis,alpha.func=alpha.func,ncores=ncores,method=method,lb=lb,ub=ub,opt=TRUE,idx.para=idx.para,pareto=TRUE,partial=TRUE,step2=FALSE,trace=TRUE);fit.result$time = proc.time() - t0
        })
