...
set.seed(random.seed)
system.time(Z.logskew.2 <- bi.simu(m=m,par=par2.2,ncores=10, model="logskew",random.seed = random.seed))
which(apply(do.call(rbind,Z.logskew.2$val),1,anyDuplicated)>0)

set.seed(random.seed)
system.time(Z.logskew.3 <- bi.simu(m=m,par=par2.3,ncores=10, model="logskew",random.seed = random.seed))
