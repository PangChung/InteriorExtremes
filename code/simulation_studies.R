...
if(model == "logskew"){
    ...
    for(i in 1:nrow(par.skew.normal)){
        ...
        for(j in 1:length(thres)){
            fit.logskew[[j]] <- fit.model(data=samples.skew.normal[[i]],loc=coord,init=par.skew.normal[i,],fixed=c(F,F,F,F,F),thres=thres[j],model="logskew",ncores=ncores,maxit=1000,method="Nelder-Mead",lb=lb,ub=ub,bootstrap=FALSE,hessian=FALSE,opt=TRUE,trace=TRUE)
