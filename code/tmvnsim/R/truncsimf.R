## simulate a truncated multivariate normal RVs
...
## simulate a truncated multivariate student-t RVs
tmvtsim <- function(nsamp, k, lower=rep(-Inf,k), upper=rep(Inf,k), imod=rep(FALSE, k), means=rep(0, k), sigma=diag(1, k),df=10)
{
	QR <- FALSE 
	if(QR)
	{
